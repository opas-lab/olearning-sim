...
from ols.ofl_commons.infrastructure.FileRepo.minio_file_repo import MinioFileRepo
...
def getPulsarObjects(manager_config=""):
    ...
    pulsar_producer = pulsar_client.create_producer(manager_config['pulsar_topic'][0])
...
def uploadS3File(
    ...
    with open(manager_config, 'r') as file:
        manager_config = yaml.load(file, Loader=yaml.FullLoader)
    s3_file_repo = S3FileRepo(manager_config)
    return s3_file_repo.upload_file(
        local_path=local_file,
        target_path=taget_file
    )


def downloadMinioFile(
    manager_config = "",
    local_file = "",
    taget_file = "",
):
    ...
    with open(manager_config, 'r') as file:
        manager_config = yaml.load(file, Loader=yaml.FullLoader)
    minio_file_repo = MinioFileRepo(manager_config)
    ...
    if file_obj.exists(): #如果存在本地文件
        ...
        return minio_file_repo.download_file(
            local_path = local_file,
            target_path = taget_file
        )
...
def uploadMinioFile(
    ...
    with open(manager_config, 'r') as file:
        manager_config = yaml.load(file, Loader=yaml.FullLoader)
    minio_file_repo = MinioFileRepo(manager_config)
    return minio_file_repo.upload_file(
        local_path=local_file,
        target_path=taget_file
    )

def listdirMinioFile(
    manager_config = "",
    prefix = ""
):
    '''
    ...
    with open(manager_config, 'r') as file:
        manager_config = yaml.load(file, Loader=yaml.FullLoader)
    minio_file_repo = MinioFileRepo(manager_config)
    return minio_file_repo.listdir_object_name(prefix=prefix)